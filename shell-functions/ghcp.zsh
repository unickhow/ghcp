# GitHub PR Cherry Pick Function for Zsh
# Source this file in your .zshrc: source /path/to/ghcp.zsh

ghcp() {
  # Enable nullglob for better array handling
...
_ghcp_check_requirements() {
  ...
  if ! git diff-index --quiet HEAD -- 2>/dev/null; then
    ...
    read "response?Continue anyway? This could lead to conflicts. [y/N] "
    if [[ ! "$response" =~ ^[Yy]$ ]]; then
...
_ghcp_confirm_operation() {
  while true; do
    read "response?Continue cherry picking these commits? [y/N] "
    case "$response" in
      [Yy]|[Yy][Ee][Ss])
        return 0
        ;;
...
_ghcp_handle_recovery() {
  ...
  if git diff-index --quiet HEAD -- 2>/dev/null; then
    ...
    read "response?Continue with remaining commits? [y/N] "
    if [[ ! "$response" =~ ^[Yy]$ ]]; then
      return 0
    fi
